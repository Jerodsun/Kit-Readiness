...
def setup_genetic_algorithm(requests, distributors, time_matrix):
    ...
    creator.create("FitnessMax", base.Fitness, weights=(1.0, -1.0))  # Maximize priority, minimize time
    ...
    toolbox.register("individual", tools.initRepeat, creator.Individual, 
                    toolbox.attr_dist, n=len(requests))
    ...
    toolbox.register("mutate", tools.mutUniformInt, low=0, up=len(distributors)-1, indpb=0.1)
...
def run_genetic_algorithm(requests, distributors, time_matrix):
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, cxpb=0.5, mutpb=0.2, 
                                  ngen=40, stats=stats, halloffame=hof, verbose=False)
...
def visualize_results(greedy_results, genetic_results, requests, distributors):
    ...
    genetic_time = sum(time_matrix[r.warehouse.name][d.name] for r, d in genetic_results)
    genetic_unfulfilled = len(requests) - len(genetic_results)
    
    # Print summary
    print("=== Results Summary ===")
    print(f"Greedy Algorithm: Priority: {greedy_priority}, Time: {greedy_time:.2f}h, Unfulfilled: {greedy_unfulfilled}")
    print(f"Genetic Algorithm: Priority: {genetic_priority}, Time: {genetic_time:.2f}h, Unfulfilled: {genetic_unfulfilled}")
...
if __name__ == "__main__":
    ...
    greedy_results, unassigned = greedy_algorithm(requests.copy(), distributors.copy(), time_matrix)
    
    print("Running genetic algorithm...")
    genetic_results = run_genetic_algorithm(requests.copy(), distributors.copy(), time_matrix)
